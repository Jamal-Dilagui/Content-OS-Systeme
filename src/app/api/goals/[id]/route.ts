...
// PATCH — update an existing goal (name, type, category, metric, targetAmount,
// currentAmount, startDate, deadline, status).
export async function PATCH(
  ...
  if (currentAmount !== undefined) {
    ...
    if (isNaN(c) || c < 0) {
      return NextResponse.json({ error: "currentAmount must be >= 0" }, { status: 400 });
    }
  ...
  if (deadline !== undefined) {
    ...
    if (isNaN(d.getTime())) return NextResponse.json({ error: "invalid deadline" }, { status: 400 });
    data.deadline = d;
  }

  // If status not explicitly provided and currentAmount/targetAmount changed,
  // recompute status from pace.
  if (status !== undefined) {
    data.status = status;
  } else if (currentAmount !== undefined || targetAmount !== undefined) {
    const startD = (data.startDate as Date) || existing.startDate;
    const endD = (data.deadline as Date) || existing.deadline;
    const t = data.targetAmount !== undefined ? Number(data.targetAmount) : existing.targetAmount;
    const c = data.currentAmount !== undefined ? Number(data.currentAmount) : existing.currentAmount;
  ...
  const updated = await db.financialGoal.update({ where: { id }, data });
